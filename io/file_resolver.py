...
# 将源文件找到翻译文字并导出json
class FileResolver:
    def __init__(self, file_path, output_path, pattern):
        self.path = file_path
    ...
    def resolve(self):
            ...
            if len(m_list) > 0:
                ...
                for item in m_list:
                    item_data = {
                        ...
                        'trans': self.transform((unicode_lines[seq][item[0]:item[1]]),unicode_lines[seq],item[0],item[1],unicode_lines,seq),
                        'auto': ''
                    }
        ...
        f = codecs.open(os.path.join(self.output_path, file_name + '-' +hashlib.md5(self.path).hexdigest()+ '-output.json'), 'w',
                        encoding="utf-8")
